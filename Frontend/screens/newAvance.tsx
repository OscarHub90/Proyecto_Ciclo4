...
mutation createAvance($descripAvance:String!, $observacion:String! $proyectoId: ID!) {
  createToDo(descripAvance: $descripAvance, proyectoId: $proyectoId) {
    id
		descripAvance
    observacion
...
const newAvanceScreen =() => {
  ...
  const [newAvance, { data, error, loading }] = useMutation(CREATE_TODO);
  if (error) {
  ...
  const onSubmit = () =>{
    newAvance({variables: {descripAvance, observacion}})
